...
const FabricReceiving = ({ selectedJob, onClose, onReceiveComplete }) => {
  ...
  const loadIssuedRolls = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/issued-rolls/${selectedJob['Lot Number']}`);
      
      console.log('📊 Issued rolls response:', response.data);
      
      if (response.data.success) {
        let activeIssuedRolls = response.data.data.filter(roll => 
        ...
        console.error('Failed to load issued rolls:', response.data.message);
      }
    } catch (error) {
  ...
  const loadReceivingHistory = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/receiving-history/${selectedJob['Lot Number']}`);
      
      if (response.data.success) {
        setReceivingHistory(response.data.data);
        console.log(`✅ Loaded ${response.data.length} receiving records`);
      }
    } catch (error) {
  ...
  const handleShadeReturn = async (shade) => {
    ...
    try {
      ...
      for (const roll of rollsForShade) {
        ...
        const response = await axios.post(`${API_BASE_URL}/store-fabric-receiving`, receivingRecord);
        ...
        if (response.data.success) {
          ...
          throw new Error(`Failed to return roll ${roll.barcodeId}: ${response.data.message}`);
        }
      }
  ...
  const handleSearchByBarcode = async () => {
    ...
    try {
      ...
      const checkResponse = await axios.get(`${API_BASE_URL}/check-barcode/${searchBarcode}?lotNumber=${selectedJob['Lot Number']}`);
      
      if (checkResponse.data.success && checkResponse.data.data.issued) {
        const issuedRollsResponse = await axios.get(`${API_BASE_URL}/issued-rolls/${selectedJob['Lot Number']}`);
        
        if (issuedRollsResponse.data.success) {
          const foundRoll = issuedRollsResponse.data.data.find(roll => roll.barcodeId === searchBarcode);
  ...
  const handleManualAdd = async () => {
    ...
    try {
      ...
      const checkResponse = await axios.get(`${API_BASE_URL}/check-barcode/${manualBarcode}?lotNumber=${selectedJob['Lot Number']}`);
      ...
      const foundRoll = issuedRollsResponse.data.data.find(roll => roll.barcodeId === manualBarcode);
      ...
      if (response.data.success) {
        ...
        alert('Failed to record return: ' + response.data.message);
      }
    } catch (error) {
    ...
    <>
                ...
                <div className="manual-entry-columns">
                  <div style={{
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      ...
                      />
                      <button onClick={handleManualAdd} disabled={submitting} style={{
                        padding: '14px',
                        background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
                        color: 'white',
                        border: 'none',
              ...
              {activeTab === 'history' && (
                <div className="history-columns">
                  <div className="section-header" style={{
                    ...
                    <h3 style={{ margin: 0, color: '#333' }}>📜 Return History</h3>
                    <p style={{ margin: '5px 0 0', color: '#666', fontSize: '14px' }}>All fabric return transactions for this lot</p>
                  </div>
                  
                  {receivingHistory.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '60px', background: '#f9f9f9', borderRadius: '12px' }}>
                      <p style={{ color: '#666' }}>No return history found for this lot.</p>
                    </div>
                  ) : (
                      ...
                      {receivingHistory.map((record, index) => (
                        <div key={index} style={{
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
                            <strong style={{ fontSize: '16px', color: '#667eea' }}>🏷️ {record.barcodeId}</strong>
                            <span style={{ fontSize: '12px', color: '#999' }}>{new Date(record.timestamp).toLocaleString()}</span>
                          </div>
                          <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '8px' }}>
                            <span style={{ fontSize: '14px', color: '#555' }}>🏭 Party: {record.cmfName || record.party || 'N/A'}</span>
                            <span style={{ fontSize: '14px', color: '#555' }}>🎨 {record.shade || 'N/A'}</span>
                            <span style={{ fontSize: '14px', fontWeight: 'bold', color: '#4caf50' }}>📥 Returned: {record.returnedWeight || record.weight} KG</span>
                            {record.cumulativeReturned && (
                              <span style={{ fontSize: '14px', color: '#2196f3' }}>📦 Total Returned: {record.cumulativeReturned} KG</span>
                            )}
                            {record.fabricUsed && (
                              <span style={{ fontSize: '14px', color: '#f57c00' }}>✂️ Used: {record.fabricUsed} KG</span>
                            )}
                            <span style={{ fontSize: '14px', color: '#666' }}>📝 {record.reason || 'Returned'}</span>
                          </div>
                          <div style={{ fontSize: '12px', color: '#999' }}>
                            Received by: {record.receivedBy || 'Production Manager'}
                          </div>
                        </div>
                      ))}
                    </div>
      ...
      {showStickerGenerator && currentReturnData && (
        <ReturnStickerGenerator
          ...
          parentBatchInfo={{
            batchNumber: `RETURN-${new Date().toISOString().split('T')[0]}`,
            batchDate: new Date().toISOString().split('T')[0],
            batchTime: new Date().toLocaleTimeString()
          }}
        />
      )}
    </>
