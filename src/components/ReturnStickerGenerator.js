...
const ReturnStickerGenerator = ({ 
  ...
  const loadNextReturnBarcodeId = async () => {
    try {
      ...
      const response = await axios.get('https://new-fabric-backend-1.onrender.com/api/google-sheets/next-return-barcode');
...
const storeReturnedRollInReceivingSheet = async (stickerData) => {
  try {
    ...
    const API_URL = 'https://new-fabric-backend-1.onrender.com/api/google-sheets/update-return-sticker';
    ...
    if (response.data.success) {
      ...
      showNotification(`⚠️ Update failed: ${response.data.message}`, 'error');
      return false;
    }
